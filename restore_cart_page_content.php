<?php
/**
 * Restore WooCommerce Cart page content to Gutenberg Blocks.
 */
...
if ($cart_page_id && $cart_page_id !== -1) {
    $original_blocks = '<!-- wp:woocommerce/cart -->
...
<h2 class="wp-block-heading has-text-align-left" style="margin-bottom:1rem">Puede que estés interesado en&hellip;</h2>
...
<h2 class="wp-block-heading has-text-align-center with-empty-cart-icon wc-block-cart__empty-cart__title">¡Tu carrito está vacío!</h2>
<!-- /wp:heading -->
...
<!-- /wp:woocommerce/cart -->';
    ...
    if ($res) {
        echo "Successfully restored Cart page to Gutenberg Blocks!\n";
    } else {
        echo "Failed to restore Cart page.\n";
    }
} else {
    echo "Cart page not found.\n";
}
